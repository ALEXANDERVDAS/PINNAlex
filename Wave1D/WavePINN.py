...
class WavePINN:
    ...
    def __init__(self, layers, operator, samples, c):
        ...
        self.ics_sample = samples[0]
        self.bcs_sample = samples[1]
        self.res_sample = samples[2]
        ...
        self.lam_ics_val = np.array(9.0) # 9 tried (Just this 9 best results)
        ...
        # Total loss
        self.loss = self.lam_r_tf * self.loss_res + self.lam_bound_tf * self.loss_bcs + self.lam_ics_tf * self.loss_ics_u + self.lam_ut_tf * self.loss_ics_u_t
        ...
        self.opt = tf.compat.v1.train.MomentumOptimizer(self.learning_rate, 0.9, use_nesterov=True) #GradientDescentOptimizer / AdamOptimizer / AdagradOptimizer / MomentumOptimizer(, 0.9)
    ...
    def train(self, iter=1, batchsize=256):
        ...
        for it in range(iter):
            ...
            if it % 100 == 0:
                ...
                print('It: %d, Loss: %.3e, Loss_res: %.3e, Loss_bcs: %.3e, Loss_u_ics: %.3e, Loss_ut_ics: %.3e,, Time: %.2f' %
                      (it, loss_value, loss_res_value, loss_bcs_value, loss_ics_u_value, loss_ics_ut_value, elapsed))
